...
class CoTACPLMetric:
    """Average Centipawn Loss for chain-of-thought moves."""

    name = "cot_acpl"

    def __init__(
        self, stockfish_path: str = "stockfish", time_limit: float = 0.05, max_tokens: int = 128
    ):
    ...
    def compute(
        ...
        if not engine or not generator or not tokenizer or not sampler:
            return {"cot_acpl": None}

        cot_uci = self._get_cot_move(session, board, generator, tokenizer, sampler)
        if not cot_uci or cot_uci == "0000":
            return {"cot_acpl": None}

        return {"cot_acpl": self._compute_acpl(engine, board, cot_uci)}

    def _get_cot_move(
        self, session: Any, board: chess.Board, generator: Any, tokenizer: Any, sampler: Any
    ) -> str | None:
        return generator.generate_move(
            session,
            board,
            tokenizer,
            sampler,
