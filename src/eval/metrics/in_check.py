...
class Top1LegalWhenInCheckMetric:
    """Compute legal move rate on check positions.

    Measures whether the model's highest-probability token is a legal move,
    but only for positions where the side to move is in check.
    """

    name = "top1_legal_when_in_check"
    ...
    def finalize(self) -> dict[str, Any]:
        if not self.buffer:
            return {"top1_legal_when_in_check": 0.0}
        return {"top1_legal_when_in_check": sum(self.buffer) / len(self.buffer)}


class AccWhenInCheckMetric:
    """Compute top-1 accuracy on check positions.
    ...
    name = "acc_when_in_check"

    def __init__(self):
        self.buffer: list[float] = []

    def compute(self, ctx: EvalContext) -> dict[str, Any]:
        if ctx.in_check and ctx.actual_token is not None:
            top1_token = torch.argmax(ctx.probs).item()
    ...
    def finalize(self) -> dict[str, Any]:
        if not self.buffer:
            return {"acc_when_in_check": 0.0}
        return {"acc_when_in_check": sum(self.buffer) / len(self.buffer)}


class IllegalMassWhenInCheckMetric:
    """Compute illegal mass on check positions.

    Sums the model's probability distribution over all illegal move tokens,
    but only for positions where the side to move is in check.
    """

    name = "illegal_mass_when_in_check"
    ...
    def finalize(self) -> dict[str, Any]:
        if not self.buffer:
            return {"illegal_mass_when_in_check": 0.0}
        return {"illegal_mass_when_in_check": sum(self.buffer) / len(self.buffer)}
