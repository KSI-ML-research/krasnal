...
@pytest.fixture
def tokenizer():
    return Tokenizer(Path("src/uci_moves.txt"))
...
class TestSpecialTokenIds:
    ...
    def test_special_tokens_list(self):
        ...
        assert SOS_ID in SPECIAL_TOKENS.values()
        assert EOS_ID in SPECIAL_TOKENS.values()
        assert PAD_ID in SPECIAL_TOKENS.values()
        assert WIN_WHITE_ID in SPECIAL_TOKENS.values()
        assert WIN_BLACK_ID in SPECIAL_TOKENS.values()
        ...
        assert STEP_BACK_ID in SPECIAL_TOKENS.values()
