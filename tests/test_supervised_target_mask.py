...
from krasnal.tokens import IS_CHECK_ID, PIECE_TYPE_MOVED_ID


def test_apply_supervised_loss_mask_matches_collate_policy():
    y = torch.tensor(
        [IS_CHECK_ID, PIECE_TYPE_MOVED_ID, 500, 999],
        dtype=torch.long,
    )
    masked = apply_supervised_loss_mask(y)

    assert masked.tolist() == [LOSS_IGNORE_INDEX, LOSS_IGNORE_INDEX, 500, 999]
