...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Run hash to inherit model/train config from (optional).",
    )
...
def main():
    ...
    if args.run:
        existing_run = find_run_by_hash(args.run, stage="pretrain")
        if existing_run:
            print(f"Continuing from existing run: {existing_run.name}")
            sys.exit(1)
        else:
            print(f"WARNING: Could not find existing pretrain run with hash {args.run}")
    ...
    from src.run_manager import update_run_config
